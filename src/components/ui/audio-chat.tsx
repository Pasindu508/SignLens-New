...
function SpeakingIndicator({ show }: { show: boolean }) {
  return (
    <div
      className={cn(
        "absolute -top-1 -right-1 bg-white rounded-full p-1.5 shadow-md",
        "transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]",
        show ? "opacity-100 scale-100" : "opacity-0 scale-0",
      )}
    >
      <div className="flex items-center justify-center gap-[2px]">
        <span className="w-[3px] bg-black rounded-full animate-wave-1" />
        <span className="w-[3px] bg-black rounded-full animate-wave-2" />
        <span className="w-[3px] bg-black rounded-full animate-wave-3" />
      </div>
    </div>
  )
}
...
function AudioWaveIcon({ isExpanded }: { isExpanded: boolean }) {
  return (
    <div
      className={cn(
        "absolute w-10 h-10 rounded-full bg-black flex items-center justify-center",
        "transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]",
    ...
    >
      <div className="flex items-center justify-center gap-[2px]">
        <span className="w-[3px] bg-white rounded-full animate-wave-1" />
        <span className="w-[3px] bg-white rounded-full animate-wave-2" />
        <span className="w-[3px] bg-white rounded-full animate-wave-3" />
      </div>
    </div>
  )
}
  ...
  return (
    <>
      ...
      <div
        ...
        className={cn(
          "relative bg-white shadow-xl shadow-black/10 overflow-hidden",
          "transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]",
      ...
      >
        ...
        <div
          className={cn(
            "absolute flex items-center gap-0.5 text-zinc-500",
            "transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]",
        ...
        >
          <div className="w-8" />
          <h2 className="text-[15px] font-semibold text-black">Voice Chat</h2>
          <button
            onClick={(e) => {
              e.stopPropagation()
              setIsExpanded(false)
        ...
        <div
          className={cn(
            "absolute left-4 right-4 h-px bg-zinc-200",
            "transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]",
          ...
          return (
            ...
            >
              <div className="relative flex flex-col items-center">
                <div
                  className="rounded-full overflow-hidden ring-[2.5px] ring-white shadow-sm transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]"
                  style={{
                    width: pos.size,
                    height: pos.size,
                ...
                <span
                  className={cn(
                    "absolute text-[13px] font-medium text-zinc-500 whitespace-nowrap",
                    "transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]",
                    isExpanded ? "opacity-100" : "opacity-0",
                  )}
                  style={{
        ...
        <button
          className={cn(
            "absolute left-4 right-4 bg-black text-white py-3.5 rounded-2xl font-medium text-[15px]",
        ...
        <p
          className={cn(
            "absolute inset-x-0 text-center text-[13px] text-zinc-500",
            "transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]",
            isExpanded ? "opacity-100" : "opacity-0",
          )}
          style={{
